...
def FEF_and_LIP(simu,path,plot_raster=False):
    prefs.codegen.target = 'numpy' 
    target_time,simu_number,t_SI,t_FS,theta_phase,gPul,g_LIP_FEF_v,target_on,runtime=simu[0],simu[1],simu[2],simu[3],simu[4],simu[5],simu[6],simu[7],simu[8]
    ...
    all_neurons_LIP, all_synapses_LIP, all_gap_junctions_LIP, all_monitors_LIP=make_full_network(syn_cond,j_tonic,gPul,t_SI,t_FS,theta_phase,theta_freq)
    ...
    all_neurons_FEF,all_synapses_FEF,all_monitors_FEF=create_FEF_full2(theta_phase,j_tonic,target_on,runtime,target_time)
...
if __name__=='__main__':
    ...
    gPul=float(sys.argv[2]) #*uA*cmeter**-2
    ...
    liste_simus=[[liste_simus[i][0],i,liste_simus[i][1],liste_simus[i][2],theta_phase,gPul,gLIP_FEFv,target_presentation,runtime] for i in range(len(liste_simus))]
