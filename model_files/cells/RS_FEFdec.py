...
eq_RS_FEFdec='''
dV/dt=1/C_RS*(-Jramp-Isyn-Igap-Iran-Iapp-IL-INa-IK-IAR+Inoise) : volt
Jramp= Jend + (2*second-t)/(2*second)* (Jbegin - Jend) : amp * meter ** -2
Jend : amp * meter ** -2
Jbegin : amp * meter ** -2

Inoise=noiseamp*randn() : amp * meter ** -2 (constant over dt)
noiseamp: amp * meter ** -2
...
Iran=sig_ranRS*randn()+s_ran*g_ranRS*(V-0*mV)*0 : amp * meter ** -2 (constant over dt)
...
sig_ranRS=0.15* mamp * cm **-2*0.5*0
