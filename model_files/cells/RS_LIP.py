...
Iran=sig_ranRS_LIP*randn()+s_ran*g_ranRS_LIP*(V-0*mV) : amp * meter ** -2 (constant over dt)
...
Iinp1=sinp*ginp_RS*(V-Vrev_inp) : amp * meter ** -2
    ...
    ginp_RS = ginp_RS_good* int(sin(2*pi*t*2*theta_freq)>=0) + ginp_RS_bad* int(sin(2*pi*t*2*theta_freq)<0) : siemens * meter **-2  
    ginp_RS_good : siemens * meter **-2
...
Iinp4=sinp1*ginp_RS1*(V-Vrev_inp) : amp * meter ** -2
    ...
    ginp_RS1 = ginp_RS_good1* int(sin(2*pi*t*2*theta_freq)>=0) + ginp_RS_bad1* int(sin(2*pi*t*2*theta_freq)<0) : siemens * meter **-2  
    ginp_RS_good1 : siemens * meter **-2
    ginp_RS_bad1 : siemens * meter **-2 
'''
...
sig_ranRS_LIP=0.15* mamp * cm **-2*0.5
g_ranRS_LIP=0.03* msiemens * cm **-2
...
if __name__=='__main__' :
    ...
    RS=NeuronGroup(1,eq_RS_LIP,threshold='V>-20*mvolt',refractory=3*ms,method='rk4')
