...
theta_freq = 4
res_folder = "Jbegin_85_Jend_15" #"jRSFEFvm_27uAcm-2" #'theta_'+str(theta_freq)+'Hz'
...
for simu in liste_simus:
    ...
    try :
        ...
        for k in range(4):
            N_hits_per_bin[int((target_time-300*ms)/hit_bin_length/ms)+k]+=hits_per_bin[k]
        ...
        for k in range(4):
            SOM_per_bin[int((target_time-300*ms)/hit_bin_length/ms)+k]+=SOM_per_target[k]
